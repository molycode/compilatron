...
void CClangUnit::OnPostInstall(std::filesystem::path const& installPath)
{
	...
	// Locate the per-target runtime directory (e.g. lib/x86_64-unknown-linux-gnu) holding the
	// shared runtimes — libomp.so, libc++.so, libunwind.so. It only exists when those runtimes
	// were built, so this self-gates: a plain build finds nothing and writes nothing.
	...
	// Clang auto-loads <bindir>/clang.cfg and <bindir>/clang++.cfg (they do not share a file),
	// so write both. The rpath is absolute: it breaks if the toolchain is moved, hence the note.
	std::string const content{ std::format(
		...
		"# LD_LIBRARY_PATH. Breaks if the toolchain is moved; delete this file to opt out.\n"
		"-Wl,-rpath,{}\n", runtimeDir.string()) };
	...
	if (wroteAny)
	{
		m_unitLog.Info(Tge::Logging::ETarget::Listeners, std::format(
			...
			"bundled runtimes ({}) without LD_LIBRARY_PATH", runtimeDir.string()));
	}
}
